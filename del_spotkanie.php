...
<body>
    <div class="content-container">
        ...
        <div class="header"><p>Usuń dane spotkania z bazy danych</p></div>
        <form action="#" method="POST">
            <div class="input-container"><label>Spotkanie: <select class="input-select" name="ID_spotkania">
                <?php
                    ...
                    if(mysqli_connect_errno()==0){
                        ...
                        while($wiersz=mysqli_fetch_array($query)){
                            $id = $wiersz['ID_spotkania'];
                            $data_godzina = $wiersz['data_godzina'];
                            $message = $id.' - '.$data_godzina;
                            if($id > 0){
        ...
        <?php
            if(isset($_POST["submit"])){
                $id = $_POST["ID_spotkania"];

                $db = mysqli_connect("localhost", "root", "", "projekt_praktyki");
                if(mysqli_connect_errno()==0){
                        $sql = "DELETE FROM spotkania WHERE ID_spotkania='$id'";
                        $db->query($sql);
                }
                mysqli_close($db);
            }
