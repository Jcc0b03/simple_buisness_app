...
<body>
    <div class="content-container">
        ...
        <div class="header"><p>Usuń dane projektu z bazy danych</p></div>
        <form action="#" method="POST">
            <div class="input-container"><label>Projekt: <select class="input-select" name="ID_projektu">
                <?php
                    $db = mysqli_connect("localhost", "root", "", "projekt_praktyki");
                    $sql = "SELECT ID_projektu, nazwa_projektu from projekty";
                    if(mysqli_connect_errno()==0){
                        $query = mysqli_query($db, $sql);
                        while($wiersz=mysqli_fetch_array($query)){
                            $id = $wiersz['ID_projektu'];
        ...
        <?php
            if(isset($_POST["submit"])){
                $id = $_POST["ID_projektu"];

                $db = mysqli_connect("localhost", "root", "", "projekt_praktyki");
                if(mysqli_connect_errno()==0){
                    $sql = "DELETE FROM projekty WHERE ID_projektu='$id'";
                    $db->query($sql);
                }
                mysqli_close($db);
            }
